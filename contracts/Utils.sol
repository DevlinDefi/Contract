...
interface IMasterChef {
    ...
    struct PoolNFTStakeInfo {
        IERC721 NFTToken; 
        uint256 allocPoint; 
        uint256 lastRewardBlock; 
        uint256 accDDTPerShare;
    }
...
interface MetaMasterChef {
    ...
    function DDTPerBlock() external view returns(uint256);
...
interface NFTStake {
    function poolInfo(uint256 pid) external view returns(PoolNFTStakeInfo memory);
...
contract Utils is Ownable{
    ...
    function CalcApy(uint256 pid, LP lpAddress,uint256 isLp, MetaMasterChef MasterChef) external view returns(uint256){
        ...
        uint256 DRG_PER_YEAR = BLOCKS_PER_YEAR.mul(MasterChef.DDTPerBlock());
    ...
    function CalcApyNFTStake(uint256 pid, IERC721 NFTAddress, NFTStake NFTStakechef) external view returns(uint256){
        PoolNFTStakeInfo memory pool = NFTStakechef.poolInfo(pid);
        uint256 poolWeight = ((pool.allocPoint).mul(amount)).div(NFTStakechef.totalAllocPoint());
        uint256 rewardTokenPrice = Tokenprice(NFTStakechef.DDT());
        uint256 DRG_PER_YEAR = BLOCKS_PER_YEAR.mul(NFTStakechef.DDTPerBlock());
        uint256 yearlyDRGRewardAllocation = (DRG_PER_YEAR.mul(poolWeight)).div(amount);
        uint256 poolLiquidityUsd = NFTAddress.balanceOf(address(NFTStakechef)).mul(100);
        if(poolLiquidityUsd == 0){
            return 0;
        }else{
            return(((yearlyDRGRewardAllocation.mul(rewardTokenPrice))).div(poolLiquidityUsd)).mul(100);
